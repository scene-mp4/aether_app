...
class _TrackersTabState extends State<TrackersTab> {
  ...
  String selectedTracker = 'Tracker 1';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        centerTitle: true,
        title: _buildTrackerDropdown(),
      ),
      body: StreamBuilder<DocumentSnapshot>(
        // Stream stays at the top level to provide data to the cards
        stream: FirebaseFirestore.instance.collection('sensor_data').doc('sensor_1').snapshots(),
        builder: (context, snapshot) {
          ...
          final String location = data['location'] ?? "938 Aurora Blvd, Cubao";
          ...
          return ListView(
            ...
            children: [
              ...
              _buildPollutantTile("PM2.5", "${data['pm25'] ?? 0}", "µg/m³", _getStatus(aqiValue), _getColor(aqiValue)),
              _buildPollutantTile("CO₂", "${data['co2'] ?? 0}", "ppm", "Normal", Colors.green),
              _buildPollutantTile("CO", "${data['co'] ?? 0}", "ppm", "Safe", Colors.green),

              const SizedBox(height: 30),

              // 3. HISTORY SECTION (Independent Sliding Widget)
              _buildSectionCard(
                title: "History",
              ...
              _buildSectionCard(
                ...
                child: Column(
                  children: [
                    _buildAdviceBox(Colors.orange, Icons.warning_amber_rounded, "Moderate Air Quality", "Sensitive groups should reduce outdoor activities."),
                    const SizedBox(height: 12),
                    _buildAdviceBox(Colors.blue, Icons.info_outline, "Health Recommendation", "Use air purifiers indoors."),
                  ],
                ),
              ),
    ...
    return DropdownButtonHideUnderline(
      child: DropdownButton<String>(
        ...
        onChanged: (String? newValue) {
          setState(() => selectedTracker = newValue!);
        },
        items: <String>['Tracker 1', 'Tracker 2', 'Tracker 3']
      ...
      child: Row(
        ...
        children: [
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text("$value $unit", style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
          ]),
          Container(
  ...
  Color _getColor(num aqi) => aqi <= 50 ? Colors.green : (aqi <= 100 ? Colors.yellow.shade800 : Colors.orange);
  String _getStatus(num aqi) => aqi <= 50 ? "Good" : (aqi <= 100 ? "Moderate" : "Unhealthy");
}

// --- CRITICAL COMPONENT: Independent state for History to stop the refresh/jump ---
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        const SizedBox(height: 20),
        Text("${tabs[selectedTabIndex]} Averages", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.black54)),
